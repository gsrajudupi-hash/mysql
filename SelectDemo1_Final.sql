...
SELECT * FROM customers;   /* selection  - select all columns/records */
...
SELECT DISTINCT contactLastName,state,city from customers where state IS NOT NULL
   ORDER BY contactLastName,state,city;
...
SELECT jobTitle,COUNT(*) AS 'Count of JobTitle' FROM employees GROUP BY jobTitle;
...
SELECT productLine,sum(quantityInStock) 'Total Quantity' 
			FROM products GROUP BY productLine HAVING SUM(quantityInStock) > 50000;
...
SELECT LENGTH('   SQL   JDBC');
SELECT LENGTH(TRIM('   SQL   JDBC'));

SELECT LENGTH('   SQL');	

SELECT SUBSTR('JDBC Programming',6,5); -- Returns part of a string - specific pos,no. of characters
...
SELECT ROUND(121.55,-2) ,ROUND(161.55,-2), ROUND(2013.567,-3),ROUND(2555.567,-3);
...
SELECT productCode, AVG(quantityOrdered*priceEach),`(AVG(quantityOrdered*priceEach)) average_item_Value
	 FROM  orderdetails
	 	GROUP BY productCode;
...
SELECT productLine FROM products WHERE buyPrice IS NULL;
